...
**로그인은 끌 수 없습니다.** 비밀번호가 없으면 앱은 아무것도 보여주지 않고 최초 설정 화면만 띄웁니다.

**① 브라우저에서 (파일 편집 없이)**
처음 접속하면 나오는 **최초 설정 화면**에서 비밀번호(8자 이상)를 정합니다. 이때 한 번만 가능합니다.
비밀번호는 PBKDF2(20만 회)로 해싱해 저장되며 평문은 남지 않습니다.

> **앱 안에서는 비밀번호를 바꿀 수 없습니다.** 로그인된 창을 누가 열어보더라도 잠금을
> 빼앗기지 않게 하기 위해서입니다. 변경은 아래 ②(환경변수)로만 하며, **환경변수 값이 항상 우선**합니다.

> 최초 설정은 **서버가 도는 컴퓨터(로컬·사설망)에서만** 허용됩니다.
> 공개된 주소에서 먼저 접속한 사람이 비밀번호를 선점하는 것을 막기 위해서입니다.
> 원격 서버라면 아래 ②로 미리 지정하세요.
...
`.env`(또는 호스팅 서비스의 환경변수)에 `APP_PASSWORD`를 넣고 재시작하면 처음부터 그 값으로 잠깁니다.
**원격 배포에는 이 방법을 쓰세요.**
...
| 항목 | 내용 |
| --- | --- |
| 로그인 강제 | 비밀번호 없이는 어떤 화면·API도 열리지 않음 (최초 설정 화면만 노출) |
| 최초 설정 제한 | 로컬/사설망에서 온 요청만 최초 비밀번호를 정할 수 있음 |
| 비밀번호 저장 | PBKDF2-HMAC-SHA256, 20만 회, 계정별 솔트 |
| 세션 | 서명된 HttpOnly 쿠키, `SESSION_DAYS`(기본 14일), https면 Secure 자동 |
| 비밀번호 변경 | **앱에서 불가** — 서버 환경변수(APP_PASSWORD)로만, 재시작 시 반영 |
| 무차별 대입 | IP당 5분 8회 실패 시 차단, 실패 응답 1초 지연 |
| CSRF | 다른 출처에서 온 상태 변경 요청(POST/PATCH/DELETE) 차단, 쿠키 SameSite=Lax |
| 응답 헤더 | CSP, X-Frame-Options: DENY, nosniff, no-referrer |
...
1. `run.bat` 을 한 번 실행해 정상적으로 뜨는지 확인
2. 창을 닫고 **`service-install.bat`** 실행 (포트를 바꾸려면 `service-install.bat 8200`)

컴퓨터를 켜면 창 없이 자동으로 시작되고, 서버가 죽으면 5초 뒤 다시 뜹니다.
기록은 `data\server.log`, 해제는 `service-uninstall.bat`.

> PC가 **절전(대기 모드)** 로 들어가면 서버도 멈춥니다.
> [설정] → [시스템] → [전원]에서 절전을 "안 함"으로 두세요.

### 리눅스 / 맥

`deploy/allone-uploader.service`(systemd), `deploy/com.allone.uploader.plist`(launchd)를 쓰세요.
파일 첫 부분에 설치 명령이 주석으로 적혀 있습니다. Docker를 쓴다면 `docker compose up -d` 만으로
`restart: unless-stopped` 가 적용됩니다.

### 어디에 두는 게 좋은가

| 방법 | 24시간 | 비용 | 비고 |
| --- | --- | --- | --- |
| 내 PC + 자동 시작 | PC를 켜둔 동안 | 무료 | 절전 끄기 필요, 외부 접속하려면 포트포워딩/터널 |
| VPS·클라우드 (Render/Fly 등 유료 소형 플랜) | 항상 | 월 몇 달러 | 디스크 붙이면 계정·기록 유지, https 자동 |
| 무료 플랜 (Render Free 등) | 접속 없으면 잠듦 | 무료 | 첫 요청이 느리고, 잠든 사이 업로드 작업이 끊길 수 있음 |
| GitHub Actions | **불가** | – | 작업당 최대 6시간, 상시 서비스 호스팅 용도가 아님 (약관 위반) |
| GitHub Pages | **불가** | – | 정적 파일만 가능. 이 앱은 파이썬 서버가 필요 |

이 앱은 SQLite와 업로드한 영상 파일을 계속 보관해야 하므로, 클라우드에 올린다면
**영구 디스크(볼륨)** 를 붙일 수 있는 곳을 고르세요.
