...
| 비용 | Starter $7 / Standard $25 / Pro $85 + 디스크 | 소형 머신 월 $3~5 수준 |
| 추천 | **처음이라면 이쪽** | 터미널이 익숙하면 |

> 무료 플랜(Render Free 등)은 접속이 없으면 잠들고 디스크도 없어서, 계정 연결과 업로드 기록이
> 재배포 때마다 사라집니다. 24시간 운영에는 맞지 않습니다.
>
> `render.yaml` 의 `plan` 은 **대시보드에서 고른 인스턴스 등급과 같아야 합니다.** 다르면
> Blueprint 를 동기화할 때 파일에 적힌 값으로 되돌아갑니다. 현재는 `pro`(4 CPU / 8GB)입니다.
> 등급을 바꿨다면 이 파일도 함께 고치세요 — `starter`(0.5 CPU) / `standard`(1 CPU) / `pro`(4 CPU).
