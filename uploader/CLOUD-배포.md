...
| 비용 | 월 $25 + 디스크 (Standard) · 최소 Starter 월 $7 | 소형 머신 월 $3~5 수준 |
| 추천 | **처음이라면 이쪽** | 터미널이 익숙하면 |

> 무료 플랜(Render Free 등)은 접속이 없으면 잠들고 디스크도 없어서, 계정 연결과 업로드 기록이
> 재배포 때마다 사라집니다. 24시간 운영에는 맞지 않습니다.
>
> `render.yaml` 은 **Standard(1 CPU / 2GB)** 로 잡혀 있습니다. 큰 영상을 여러 채널로 동시에
> 내보낼 때 속도가 여기서 갈립니다. 비용을 줄이려면 `plan: starter`(0.5 CPU, 월 $7)로 낮춰도
> 동작은 합니다 — 대용량 영상 전송이 그만큼 느려질 뿐입니다.
